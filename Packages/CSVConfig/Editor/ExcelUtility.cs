...
namespace DFDev.CSVConfig.Editor
{
    public class ExcelUtility
    {
        ...
        {
            FileStream stream = new FileStream(excelFile, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
            stream.Seek(0, SeekOrigin.Begin);
            IExcelDataReader excelReader = ExcelReaderFactory.CreateOpenXmlReader(stream);
            _resultSet = excelReader.AsDataSet();
        }
        ...
        public List<T> ConvertToList<T>()
        {
            ...
            if (_resultSet.Tables.Count < 1)
                return null;
            //默认读取第一个数据表
            DataTable mSheet = _resultSet.Tables[0];
        ...
        public void ConvertToCSV(string CSVPath, Encoding encoding)
        {
            ...
            //默认读取第一个数据表
            DataTable sheet = _resultSet.Tables[0];
            ...
            {
                if (string.IsNullOrWhiteSpace(sheet.Rows[i][0].ToString()))
                    continue;
                for (int j = 0; j < colCount; j++)
                {
                    //使用","分割每一个数值
